...
class PidFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        openapsma_run.setOnClickListener {
            PidPlugin.getPlugin().invoke("OpenAPSAMA button", false)
        }
    }
    ...
    private fun updateGUI() {
        ...
        PidPlugin.getPlugin().lastDetermineBasalAdapterAMAJS?.let { determineBasalAdapterAMAJS ->
        ...
        if (PidPlugin.getPlugin().lastAPSRun != 0L) {
            openapsma_lastrun.text = DateUtil.dateAndTimeFullString(PidPlugin.getPlugin().lastAPSRun)
        }
        PidPlugin.getPlugin().lastAutosensResult?.let {
